...
#                first_data = set_factors(first_data)
...
#                second_data = set_factors(second_data)
                ...
                final_data = pd.merge(fourth_data,first_data,how='left',on='GICODE')
                final_data['return'] = final_data['ADJ_PRC_x']/final_data['ADJ_PRC_y']
                ...
                model = model.fit(np.array(final_data['ROE_x_y']).reshape(-1,1),np.array(final_data['return']).reshape(-1,1))
                model.coef_
                model.intercept_
                y_new = pd.DataFrame(model.predict(np.array(final_data['ROE']).reshape(-1,1)))
