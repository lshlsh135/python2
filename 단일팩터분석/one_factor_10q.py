# -*- coding: utf-8 -*-
"""
Created on Wed Nov 28 13:31:09 2018
...
class QVGSM_VALUE:
    ...
    def __init__(self,stock_num,raw_data,rebalancing_date,kospi_day,daily_return,gross_col_loc,profit_col_loc,value_col_loc,cpi_data,oscore,momentum):
       ...
       for i in range(1,11): # 5분위를 저장해야 하기 때문에 모든 변수를 5개씩 선언해준다.
...
#                first_data = first_data[first_data['MARKET_CAP']>100000000000]
                ...
                for i in range(1,10):
                    locals()['q_{}'.format(i)]=first_data[(first_data[factor]>first_data[factor].quantile(1-0.1*(i)))
                                                        &(first_data[factor]<=first_data[factor].quantile(1 - 0.1*(i-1)))]
