# -*- coding: utf-8 -*-
"""
Created on Mon Jul 15 18:01:03 2019
...
#DATA를 가져온다!!
kospi = pd.read_sql("""select lead(trd_date,1) over(partition by gicode order by trd_date) trd_date,trd_date as trd_date_old, gicode, co_nm,iskosdaq,pretax_ni_ttm, wics_mid,wics_big,cap_size,EQUITY,CFO_TTM,cash_div_com,SALES_TTM,lead(ADJ_NI_12M_FWD,1) over(partition by gicode order by trd_date) ADJ_NI_12M_FWD,lead(ADJ_NI_12M_FWD,2) over(partition by gicode order by trd_date) ADJ_NI_12M_FWD_2lead,lead(market_cap,1) over(partition by gicode order by trd_date) market_cap,lead(market_cap,2) over(partition by gicode order by trd_date) market_cap_2lead,asset,asset - equity as liab,ni,liq_equity,liq_debt,(NI-lag(ni,3) over(partition by gicode order by trd_date))/ (ABS(ni) + abs(lag(ni,3) over(partition by gicode order by trd_date))) as chin,lag(ni,12) over(partition by gicode order by trd_date) as ni_1y, gross_profit_ttm ,
                    ...
                    lead(adj_prc,1) over(partition by gicode order by trd_date) adj_prc,lead(adj_prc,2) over(partition by gicode order by trd_date) adj_prc_2lead,
                    ...
                    lead(float_weights,1) over(partition by gicode order by trd_date) float_weights,lead(float_weights,2) over(partition by gicode order by trd_date) float_weights_2lead
...
daily_return = pd.read_sql("""select trd_date, gicode, co_nm, adj_prc_d, lag(adj_prc_d, 252) over(partition by gicode order by trd_date) adj_prc_d_252 from kospi_daily_stock_20190628 """,con=connection).drop_duplicates()
...
#free_float = pd.read_sql("""select * from float_weights_20190628""",con=connection) # 코스피 일별 종가
#raw_data = pd.merge(raw_data,free_float,how='inner',on=['TRD_DATE','GICODE']
...
#raw_data['ROE_mv_8'] = raw_data.groupby('gicode')['sales_pct_chg'].apply(lambda x: x.rolling(8).std())
