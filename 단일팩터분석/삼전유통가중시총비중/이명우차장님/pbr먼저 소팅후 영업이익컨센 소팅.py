# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 13:56:21 2019
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 17:23:37 2019
...
raw_data = pd.read_csv('dg_csv_kospikosdaq_operating_20190703.csv',engine='python')
...
raw_data['ope_shift'] = raw_data.groupby(['gicode'])['ope'].shift(1)
...
raw_data['ope_surprise'] = abs((raw_data['ope'] - raw_data['ope_consen'])/raw_data['ope_consen'])
factor = ['1/pbr']
...
#        first_data = first_data.loc[:,~first_data.columns.duplicated()] # 중복 columns를 제거한다
        ...
        third_data = pd.DataFrame(third_data.loc[:,['trd_date','gicode','ope_surprise']])
        third_data2 = third_data.groupby('gicode').count().reset_index()
        third_data2 = third_data2[third_data2['ope_surprise']==3]
...
#        first_data = first_data[(first_data['ope_surprise_x'].notnull())&(first_data['ope_surprise'].notnull())]
#        first_data['ope_surprise'] = first_data['ope_surprise_x'] + first_data['ope_surprise']
...
#        
        ...
        for i in factor2:
            ...
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][locals()['q_{}'.format(i)][i+'_y']>=locals()['q_{}'.format(i)][i+'_y'].quantile(2/3)]
            
            locals()['q_{}'.format(i)] = pd.merge(locals()['q_{}'.format(i)],third_data3,how='inner',on='gicode')
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][(locals()['q_{}'.format(i)]['ope_surprise_x'].notnull())&(locals()['q_{}'.format(i)]['ope_surprise'].notnull())]
            locals()['q_{}'.format(i)]['ope_surprise'] = locals()['q_{}'.format(i)]['ope_surprise_x'] + locals()['q_{}'.format(i)]['ope_surprise']
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)].rename(columns={'trd_date_y_x':'trd_date_y'})
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][locals()['q_{}'.format(i)]['ope_surprise']<=locals()['q_{}'.format(i)]['ope_surprise'].quantile(1/3)]
...
#                locals()['q_{}'.format(i)]['float_weights'] = (1 - samsung_float_weights)/(stock_num)
...
#                locals()['q_{}'.format(i)].loc[locals()['q_{}'.format(i)]['gicode']!='A005930','float_weights'] = (1 - samsung_float_weights)/(stock_num)
            print(len(locals()['q_{}'.format(i)]))
            z = pd.concat([z,locals()['q_{}'.format(i)][['trd_date_x','co_nm']]])
