# -*- coding: utf-8 -*-
"""
Created on Thu May 23 15:48:10 2019
...
daily_return = pd.concat([pd.read_sql("""select * from kospi_daily_stock_20190430 """,con=connection),pd.read_sql("""select * from kosdaq_daily_stock_20190430 """,con=connection)],axis=0,ignore_index=True).drop_duplicates()
...
raw_data = pd.read_csv('dg_csv_kospikosdaq_20190430.csv',engine='python')
float_cap_data = pd.read_excel('kospi200_weight.xlsx')
float_cap_data['trd_date'] = float_cap_data['trd_date'].astype(str)
#raw_data['trd_date'] = pd.DataFrame(pd.to_datetime(raw_data['trd_date']))
...
raw_data['adj_dps_shift'] = raw_data.groupby(['gicode'])['adj_dps'].shift(1)
...
stock_num = 25
for n in range(start_n,col_length-1): 
    if rebalancing_date.iloc[n,0][5:7] =='02':
        ...
        first_data['adj_prc'] = first_data['adj_prc_shift']
        first_data['float_cap'] = first_data['float_cap_shift']
...
#            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][locals()['q_{}'.format(i)]['rnk']<=stock_num+1]
            locals()['q_{}'.format(i)]['rtn_rnk'] = locals()['data_{}'.format(i)].loc[:,'1m_rtn'].rank(method='first',ascending = False)
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][(locals()['q_{}'.format(i)]['rnk']>5)&(locals()['q_{}'.format(i)]['rnk']<=stock_num+5)]
            locals()['q_{}'.format(i)] = locals()['q_{}'.format(i)][locals()['q_{}'.format(i)]['rtn_rnk']>=60]
...
#                q_temp.loc[q_temp.loc[:,'FLOAT_WEIGHTS'].isnull(),'FLOAT_WEIGHTS'] = 0.22
                ...
                locals()['q_{}'.format(i)].loc[locals()['q_{}'.format(i)]['gicode']!='A005930','float_weights'] = (1 - samsung_float_weights)/(len(locals()['q_{}'.format(i)])-1)
            ...
            rst_rtn_d.loc[(rst_rtn_d['trd_date']==rst_rtn_d.loc[0,'trd_date'])&(rst_rtn_d['gicode']!='A005930'),'rtn_d'] = locals()['end_wealth_{}'.format(i)]  * (1-samsung_float_weights)/(len(locals()['q_{}'.format(i)])-1)
